...
{
    const auto& maneuver_obstacle_ids =
        !state.obstacle_ids.empty()
            ? state.obstacle_ids
            : std::vector<int>{ state.obstacle_id };

    for( const int obstacle_id : maneuver_obstacle_ids )
    {
        if( obstacle_id < 0 )
        {
            continue;
        ...
        {
            // Not visible this cycle: hold the latched maximum, do not shrink.
            continue;
        }

        const double length =
            std::max( params.min_vehicle_dimension,
        ...
        auto envelope_it =
            std::find_if(
                state.tracked_obstacles.begin(),
                state.tracked_obstacles.end(),
                [&]( const planner::TrackedObstacleEnvelope& envelope )
                {
                    return envelope.participant_id == obstacle_id;
                } );

        if( envelope_it == state.tracked_obstacles.end() )
        {
            planner::TrackedObstacleEnvelope envelope;
            envelope.participant_id = obstacle_id;
            envelope.max_length = length;
            envelope.max_width = width;
            state.tracked_obstacles.push_back( envelope );
        }
        else
        {
            envelope_it->max_length = std::max( envelope_it->max_length, length );
            envelope_it->max_width = std::max( envelope_it->max_width, width );
        }
    }
}

std::optional<planner::TrackedObstacleEnvelope>
tracked_obstacle_for(
    const ActiveAvoidanceState& state,
    int participant_id )
{
    const auto envelope_it =
        std::find_if(
            state.tracked_obstacles.begin(),
            state.tracked_obstacles.end(),
            [&]( const planner::TrackedObstacleEnvelope& envelope )
            {
                return envelope.participant_id == participant_id;
            } );
